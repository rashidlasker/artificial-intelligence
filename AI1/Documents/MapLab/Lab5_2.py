...
def getDictFromFile(name):
    ...
    fileName = 'E:/Documents/MapLab/' + name + '.txt'
...
#---------------------------------------------------------------------------------------------------
def findShortestPath(firstStation, finalStation, neighborDict, nodesDict):
    queue = [(0 + findDistance(firstStation, finalStation, nodesDict), firstStation,[], 0)]
    CLOSED = dict()
    popCount = 0
    ...
    while queue:
        ...
        for currentChild in childrenList:
            ...
            newChild = (currentG + findDistance(currentChild, finalStation, nodesDict), currentChild, currentPath, currentG)
            index = 0
...
#-----------------------------------------------<Main>----------------------------------------------
def main():
    neighborDict = getDictFromFile('romNeighborsDict')
    nodesDict = getDictFromFile('romNodesDict')
    firstStation = 'Arad'
    finalStation = 'Bucharest'
    #checkStation(firstStation, neighborDict)
    #checkStation(finalStation, neighborDict)
    findShortestPath(firstStation, finalStation, neighborDict, nodesDict)
